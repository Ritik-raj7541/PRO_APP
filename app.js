...
app.get("/user/:userId", async function (req, res) {
  ...
  let idAndName = [];
  const foundUser = await User.findById(searchedUser);
  for (const element of foundUser.post) {
    const docs = await Post.findById(element);
    if (docs) {
      idAndName.push({
        id: element,
        name: docs.title,
  ...
  res.render("user", { user: foundUser, posts: idAndName });
});
